...
if errors:
    print("Errors detectats. No es poden calcular les estadístiques.")
else:
    ...
    for any, estacio in sorted(estacio_mes_plujosa_per_any.items()):
        print(f"  {any}: {estacio}")
    print(f"Año más lluvioso: {any_mes_plujos} ({promig_anual_per_pais[any_mes_plujos]:.2f} litres)")
    print(f"Año más seco: {any_mes_sec} ({promig_anual_per_pais[any_mes_sec]:.2f} litres)")
